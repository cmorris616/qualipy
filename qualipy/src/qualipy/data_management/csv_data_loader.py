...
class CsvDataLoader(DataLoader):
    def load_data(self, **kwargs):
        """
        Loads the data from the specified CSV file.  A header row is
        required.  The column headers must match the properties of
        ...
        - model_class: the fully qualified name of the model class represented
                        by the data being read.  If this is ommitted, a list of
                        dictionary objects will be returned with the first row
                        of the file being treated as the keys.

        - quoting: the type of quoting used in the file.  csv.QUOTE_NONNUMERIC is the
                    default and recommended value.  Using other quoting may affect
                    data types as the data is read.
